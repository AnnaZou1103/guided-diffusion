#!/bin/bash
# Auto-resubmit training script for ImageNet fine-tuning on ArtBench style
# This script automatically resubmits training jobs until completion
# Usage: bash train_imagenet_finetune_loop.sh [style_name] [artbench_images_dir] [pretrained_model_path] [max_iterations]
# Example: bash train_imagenet_finetune_loop.sh impressionism
# Example: bash train_imagenet_finetune_loop.sh impressionism /path/to/artbench_images /path/to/256x256_diffusion_uncond.pt 50

STYLE_NAME=${1:-"surrealism"}
ARTBENCH_IMAGES_DIR_ARG=${2:-""}  # Optional: ArtBench images directory (if not provided, will auto-detect)
PRETRAINED_MODEL_ARG=${3:-""}  # Optional: Pretrained model path (if not provided, will use default ImageNet model)
MAX_ITERATIONS=${4:-50}  # Maximum resubmissions (about 100 hours)
...
if [ ! -d "$SCRIPT_DIR" ] || [ ! -f "$SCRIPT_DIR/train_imagenet_finetune.sh" ]; then
    echo "Error: Cannot find train_imagenet_finetune.sh in $SCRIPT_DIR"
    echo "Current directory: $PROJECT_ROOT"
    echo "Please run from project root directory: cd /path/to/guided-diffusion && bash run/train_imagenet_finetune_loop.sh"
    exit 1
fi

LOG_DIR="${PROJECT_ROOT}/logs/artbench_imagenet_${STYLE_NAME}"
TARGET_STEPS=200000
...
if [ -n "$PRETRAINED_MODEL_ARG" ]; then
    # Use provided path (convert to absolute if relative)
    if [[ "$PRETRAINED_MODEL_ARG" = /* ]]; then
        PRETRAINED_MODEL="$PRETRAINED_MODEL_ARG"
    else
        PRETRAINED_MODEL="$(cd "$(dirname "$PRETRAINED_MODEL_ARG")" && pwd)/$(basename "$PRETRAINED_MODEL_ARG" 2>/dev/null || echo "${PROJECT_ROOT}/${PRETRAINED_MODEL_ARG}")"
    fi
else
...
echo "=========================================="
echo "Auto-resubmit Training: ImageNet Fine-tuning on ${STYLE_NAME}"
...
while [ $iteration -lt $MAX_ITERATIONS ]; do
    ...
    echo "Training script: $SCRIPT_DIR/train_imagenet_finetune.sh"
    
    # Use absolute path for sbatch to avoid SLURM temp directory issues
    TRAIN_SCRIPT_PATH="${SCRIPT_DIR}/train_imagenet_finetune.sh"
    if [ ! -f "$TRAIN_SCRIPT_PATH" ]; then
        echo "Error: Training script not found: $TRAIN_SCRIPT_PATH"
        exit 1
    fi
    
    # Pass style_name, dataset_dir, and pretrained_model to train_imagenet_finetune.sh
    ...
    echo "Job ID: $JOB_ID"
    echo "Job output: artbench_imagenet_train_${JOB_ID}.out"
    echo "Job error: artbench_imagenet_train_${JOB_ID}.err"
    ...
    ERROR_LOG_DIR="${PROJECT_ROOT}/logs/artbench_imagenet_${STYLE_NAME}_errors"
    ...
    JOB_OUT_FILE="artbench_imagenet_train_${JOB_ID}.out"
    JOB_ERR_FILE="artbench_imagenet_train_${JOB_ID}.err"
    ...
    if [ "$JOB_FAILED" = true ]; then
        ...
        echo "  sbatch run/train_imagenet_finetune.sh $STYLE_NAME $ARTBENCH_IMAGES_DIR $PRETRAINED_MODEL"
        exit 1
    fi
    ...
    if [ -f "$JOB_ERR_FILE" ]; then
        ...
        if [ "$ERROR_SIZE" -gt 0 ]; then
            ...
            tail -n 5 "artbench_imagenet_train_${JOB_ID}.err"
        fi
    fi
